import { sports, entities, comparisons } from "./seed-data.mjs";
...
for (const [sport, name, slug, p, sec] of entities)
  sql += `insert into entities (sport_id, name, slug, primary_color, secondary_color) select id, ${q(name)}, ${q(slug)}, ${q(p)}, ${q(sec)} from sports where slug=${q(sport)} on conflict (slug) do nothing;\n`;
...
console.log("comparisons:", comparisons.length, "entities:", entities.length, "bytes:", sql.length);
